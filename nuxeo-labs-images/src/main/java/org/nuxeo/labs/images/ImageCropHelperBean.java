...
/**
 * Display "Crop image" UI in a fancybox.
 * The bean sets up information about the picture and its size, to make it fit
 * in the fancybox, while using a small/medium image (void using the original
 * multi-mega bytes image). Possibly, the final width/height will be bigger than
 * the image used, so the browser will scale it.
 *
 * When the user clicks "Crop" button, the crop itself is managed by the JavaScript
 * not by this Bean (see resources/web/nuxeo.war/scripts/nuxeo-labs-image-cropper.js)
 *
 * @since 5.9.6
 */
...
public class ImageCropHelperBean implements Serializable {
    ...
    public void initialize() throws Exception {
        try {
            ...
            ArrayList<Map<String, Object>> views = (ArrayList<Map<String, Object>>)
                                                  currentDocument.getProperty("picture", "views");
            ...
            if(views != null) {
                ...
                // Either we have no views or none of them are < kMAX_WIDTH, which
                // is very, very unlikely. Result will be weird anyway.
                if(imageWidth == 0) {
                    // We do have a problem
                    log.warn("No  picture view found which is less than " + kMAX_WIDTH + "x" + kMAX_HEIGHT);
                }
            }

            //log.warn("VIEW TO USE: " + imageViewName);
            //log.warn("Before: " + imageWidth + "x" + imageHeight);
            _updateDimensions();
            //log.warn("After: " + imageWidth + "x" + imageHeight);
    ...
    /*
     * Check width/height and update them so that:
     *  - If they are smaller than kMAX_WIDTH/kMAX_HEIGHT, and original width/height are greater,
     *    it means we can resize the picture to kMAX_WIDTH/kMAX_HEIGHT. The picture used will then
     *    be scaled, but it will be more comfortable for the user.
     *
     *  - If they are greater than kMAX_WIDTH/kMAX_HEIGHT, then picture is resized to the max. values.
     */
