...
/**
 * Crops the image embedded in a document:
 *      - Uses the main file ("file:content")
 *      - Uses the crop coordinates passed as parameters
 *      - pictureWidth/pictureHeight are the size of the picture used to
 *        build the crop coordinates.
 *      - Scale them to fit the original image size if pictureWidth/pictureHeight
 *        are not 0. If not passed, the operation assumes the original
 *        "file:content" image size is used
 *      - If required, a version of the document is first created
 *      - Then, the cropped picture is stored in "file:content", replacing the
 *        previous one.
...
public class ImageCropInDocumentOp {
    ...
    public static final Log log = LogFactory
            .getLog(ImageCropInDocumentOp.class);
    ...
    public DocumentModel run(DocumentModel inDoc) throws ClientException {
        ...
        if (!incrementVersion.equalsIgnoreCase("none") && inDoc.hasFacet(FacetNames.VERSIONABLE)) {
            VersioningOption vo;
        ...
        targetFileName = ConversionUtils.updateTargetFileName(pictureBlob, targetFileName, targetFileNameSuffix);
        ...
        Blob croppedBlob = ConversionUtils.convert("imageCropping", pictureBlob, params, targetFileName);
