 ...
 * original blob is returned unchanged. pictureWidth/pictureHeight is the size
 * of the picture used for cropping. The coordinates of the crop will be scaled
 * to fit the original picture size. So, if original picture is 4000x2000, the
 * picture used for cropping is 1000x500 (4 time smaller), and the crop is 10,
 * 10, 200, 200, the final crop on the original picture will be 40, 40, 800,
 * 800, so there will be no changes for the use: The final crop will look the
 * same as what he/she selected in the UI (but bigger)
 * 
 ...
 * added to the target file name before the extension (So, if the file name is
 * "mypict.jpg" and the suffix is "-copy", the target file name will be
 * "mypict-copy.jpg")
 * 
...
public class ImageCropOp {
    ...
    public static final Log log = LogFactory.getLog(ImageCropOp.class);
    ...
    public Blob run(Blob inBlob) {
        ...
        if (pictureWidth > 0 && pictureHeight > 0) {
            ImageInfo imgInfo = Framework.getService(ImagingService.class).getImageInfo(
                    inBlob);
        ...
        Blob result = ConversionUtils.convert("imageCropping", inBlob, params,
                targetFileName);
        return result;
    }
}
