...
/**
 *  In OSGI-INF/extensions/conversions-contrib.xml, you can find:
 *      - The contribution to the ConversionService, using the
 *        org.nuxeo.labs.images.GenericImageMagickConverter class
 *      - And the contribution to the CommandLineExecutorComponent
 *
 *  So, to summarize, we have:
 *      - Converters with unique names ("overlaying", "changeFormat", ...)
 *      - Which are bound to commandLine ("imageOverlay", "changeFormat", ...)
 *      - At runtime, the parameters are packed in a map and passed to the the
 *        command line executor
 *
 *  IMPORTANT: This means the parameters must match what the command line expects.
 *             For example, for "overlay", a parameter named "textSize" must
 *             exist in the context, since the "convert" commandLine uses it
 *             in its parameterString<> tag:
 *                  . . . -pointsize #{textSize} . . .
 *
 */
